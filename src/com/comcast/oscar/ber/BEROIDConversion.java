...
public class BEROIDConversion {
  ...
  public Map<String, String> toMap() {
    ...
    // Make sure it is a OCTET String and a HexString with : and is it contains NON PlainText ASCII
    // Char
    // mgarcia - 140224 - Added Logic to figure out a HexString vs. OctetString in some conditions
    // from Packet ACE Tool
    if ((vbBER.getSyntax() == BinaryConversion.byteToUnsignedInteger(BER.OCTETSTRING))
        &&

        /* Checks to see if the string is in DEXTER HEX FORMAT */
        /*Checks to see if there is a HEX String of 1 byte */
      ...
      mssReturn.put(
          DATA_TYPE, Integer.toString(BinaryConversion.byteToUnsignedInteger(BERService.HEX)));

      mssReturn.put(VALUE, vbBER.getVariable().toString());

    } else if ((vbBER.getSyntax() == BinaryConversion.byteToUnsignedInteger(BER.OCTETSTRING))
        && ((HexString.isHexString(vbBER.getVariable().toString()))
            && (vbBER.getVariable().toString().matches("(.*:.*)+?")))
        && (!HexString.verifyAsciiPlainText(
            HexString.toByteArray(vbBER.getVariable().toString())))) {
      ...
      mssReturn.put(VALUE, vbBER.getVariable().toString());
        ...
        && (HexString.containAsciiWhiteSpace(
            HexString.toByteArray(HexString.asciiToHex(getOidValue()))))) {

      if (logger.isDebugEnabled())
        logger.debug(
            "BEROIDConversion.toMap() - Found a ASCII Plain Text Characters: \""
                + getOidValue()
                + "\"");
