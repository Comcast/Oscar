...
public class PacketCableCompiler extends TlvBuilder {
  ...
  /**
   * this methods creates the SHA-1 Hash required for BASIC.1 file integrity check based on the
   * PacketCable Version
   */
  public void commit() {
    ...
    try {
      bTlvFileMarker =
          stripTlv(
              BinaryConversion.byteToUnsignedInteger(PacketCableConstants.FILE_MARKER),
              super.toByteArray());
    ...
    if (logger.isDebugEnabled())
      logger.debug(
          "PacketCableCompiler.finalize() = STRIP-FILE-MRK:        "
              + new HexString(bTlvFileMarker).toString());
    ...
    if (logger.isDebugEnabled())
      logger.debug(
          "PacketCableCompiler.finalize() = SET-FILE-MRK:          "
              + new HexString(bTlvFileMarker).toString());
    ...
    if (logger.isDebugEnabled())
      logger.debug(
          "PacketCableCompiler.finalize() = SHA1-HASH-HEX:         "
              + new HexString(bSHA1).toString());
    ...
    if (logger.isDebugEnabled())
      logger.debug(
          "PacketCableCompiler.finalize() = STRIP-FILE-MRK-HEX:    "
              + new HexString(bTlvFileMarker).toString());
    ...
    if (logger.isDebugEnabled())
      logger.debug(
          "PacketCableCompiler.finalize() = ADD-SHA1-OID-HASH-HEX: "
              + new HexString(bTlvFileMarker).toString());
  ...
  public static byte[] stripTlv(int iType, byte[] bTlvByteArray) throws TlvException {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "PacketCableCompiler.stripTlv(i,b) -> StripType: "
              + iType
              + " - ByteArray Size: "
              + bTlvByteArray.length);
      logger.debug(
          "PacketCableCompiler.stripTlv(i,b) -> Hex: "
    ...
    while (iIndex < bTlvByteArray.length) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "+----------------------------------------------------------------------------+");
        logger.debug(
            "PacketCableCompiler.stripTlv(i,b)"
                + " -> Type: "
      ...
      // Check for the DoubleByte Length TLV - Total Hack!!! - Next Version, I will use the
      // Dictionary
      if (bTlvByteArray[iIndex] == PacketCableConstants.SNMP_TLV_64) {
        ...
        baosMultiByteLength.write(bTlvByteArray, (iIndex + TLV_LENGTH_POS_OFFSET), 2);
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "PacketCableCompiler.stripTlv(i,b) -> Found: Type64 - Index: "
                  + iIndex
                  + " - TypeLength: "
                  + iTypeLength);
        }
      ...
      if (BinaryConversion.byteToUnsignedInteger(bTlvByteArray[iIndex]) != iType) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "PacketCableCompiler.stripTlv(i,b) -> Copying Bytes to ByteArray - Index: "
                  + iIndex
                  + " - TypeLength: "
                  + iTypeLength);
          logger.debug(
      ...
      if (logger.isDebugEnabled())
        logger.debug(
            "PacketCableCompiler.stripTlv(i,b) -> NextIndex: "
                + iIndex
                + " - ByteArray Size: "
                + bTlvByteArray.length);
    }
  ...
  public static List<HexString> getTopLevelTlvToHexStringList(byte[] baTlv) throws TlvException {
    ...
    for (int iIndex = 0; iIndex < baTlv.length; ) {
      ...
      // Build ByteArray
      baosTopLevelTLV.write(
          baTlv, iIndex, (iLength + iNumByteLength + TlvBuilder.TLV_TYPE_OVERHEAD));
      ...
      if (logger.isDebugEnabled())
        logger.debug(
            "PacketCableCompiler.getTopLevelTlvToHexStringList() - TLV -> " + hsTLV.toString(":"));
  ...
  public static TlvBuilder stripFileMarkers(byte[] baTLV) throws NullPointerException {
    ...
    try {
      baTlvTemp =
          stripTlv(BinaryConversion.byteToUnsignedInteger(PacketCableConstants.FILE_MARKER), baTLV);
  ...
  private byte[] insertSHA1Hash(byte[] bTlvArray, byte[] bSHA1) throws IOException {
    ...
    if (iPacketCableVersion == ConfigurationFileTypeConstants.PKT_CABLE_10_CONFIGURATION_TYPE) {
      ...
      tbTlvSHA1.add(
          PacketCableConstants.SNMP_TLV_11,
