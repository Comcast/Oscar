...
public class NetSNMP extends ArrayList<String> {
	 ...
	 */
	public DottedToTextualOIDMap toTextualOID() {
	...
	public static String toTextualOID(String sOID) {
		...
		return runSnmpTranslate(sSnmpTranslate).get(sOID);
	 ...
	 * @return
	 */
	private DottedToTextualOIDMap runSnmpTranslate(String sSnmpTranslateCMD) {

		boolean localDebug = Boolean.FALSE;
		
		DottedToTextualOIDMap dttom = new DottedToTextualOIDMap(super.);
		...
		try {
			while ((sStd_IO = stdInput.readLine()) != null) {
				...
				if (!sStd_IO.isEmpty()) {
					/* Add to Mapping */
					dttom.put(super.get(iIndex++),sStd_IO);
				}
