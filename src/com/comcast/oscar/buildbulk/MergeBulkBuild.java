...
public class MergeBulkBuild {
  ...
  public static final Boolean VERBOSE_DEFAULT_OUTPUT_ON =
      ConfigurationFileExport.EXPORT_DEFAULT_TLV;
  ...
  private ArrayList<ArrayList<ConfigurationFile>> alalcf =
      new ArrayList<ArrayList<ConfigurationFile>>();
  ...
  public MergeBulkBuild(
      int iConfigurationFileType, File fOutputDir, boolean fOutputType, String sSharedSecret) {
  ...
  /**
   * @param boolDefaultVerboseMode VERBOSE_DEFAULT_OUTPUT_ON include Default TLV when Textual Output
   *     is Selected
   */
  ...
  public void start() {
    ...
    if (logger.isDebugEnabled()) logger.debug("MergeBulkBuild.start() -> Files: " + alcfTemp);
    ...
    for (ConfigurationFile cf : alcfTemp) {
      ...
      if (boolOutputType) {
        cf.setConfigurationFileName(
            fOutputDir + File.separator + cf.getConfigurationFileName() + FILENAME_EXTENSION);
        cf.writeToDisk();
        /*Text*/
      } else {
   ...
   */
  private ArrayList<ConfigurationFile> mergerDirectories(
      ArrayList<ConfigurationFile> alcf1, ArrayList<ConfigurationFile> alcf2)
      throws MergeBulkBuildException {
