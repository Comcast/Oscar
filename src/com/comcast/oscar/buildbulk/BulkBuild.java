...
public class BulkBuild {
	 ...
	 */
 	public BulkBuild (File fInputDirectory , File fOutputDirectory, int iCompilingMethod , String sSharedSecretKey) {
	 ...
	 * @param cti
	 */
	public BulkBuild (File fInputDirectory , File fOutputDirectory, int iCompilingMethod , String sSharedSecretKey, CommonTlvInsertions cti) {
		this.fInputDirectory = fInputDirectory;
		this.fOutputDirectory = fOutputDirectory;
		this.iCompilingMethod = iCompilingMethod;
		this.sSharedSecretKey = sSharedSecretKey;
	...
	public boolean textToBinary (File fTextInput , File fBinOutput , String sSharedSecretKey) {
		...
		ConfigurationFile cf = new ConfigurationFile(cfi.getConfigurationFileType(),cfi.getTlvBuilder(),sSharedSecretKey);
	...
	public boolean binaryToBinary(File fBinInput , File fBinOutput , String sSharedSecretKey) {
		...
		try {
			cf = new ConfigurationFile(cfe.getConfigurationFileType(),cfe.getTlvBuilder(),sSharedSecretKey);
		} catch (TlvException e) {
			e.printStackTrace();
		}
	...
	/**
	 * This method will do a Specification Check before adding TLVs
	 * this Method assumes that each TlvBuilder has only 1 TopLevel TLV
	 * 
	 * @param cti
	 * @param cf
	 */
	private void commonTlvInsertionsToConfigurationFile (CommonTlvInsertions cti , ConfigurationFile cf) {		
