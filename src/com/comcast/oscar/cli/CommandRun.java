...
public class CommandRun {
  ...
  /**
   * Checks all the commands from the user. Order is IMPORTANT. Do not rearrange without full
   * understanding.
   *
   * @param args
   */
  public void run(String[] args) {
    ...
    try {
      ...
      if (line.hasOption("x")) {
        ...
        if (comInput.hasInput()) {
          if (comInput.isBinary()) {
            ...
            comHexDisplay.printHexDisplayFromText(
                comInput.getInput(), comSpecification.getConfigurationFileType());
          }
        } else {
          System.err.println(Input.ERROR);
          System.exit(1);
      ...
      if (line.hasOption("j")) {
        if (comInput.hasInput()) {
          if (comInput.isBinary()) {
            comJSONDisplay.printJSONDisplayFromBinary(
                comInput.getInput(),
                comSpecification.getTlvDisassemble(),
                comSpecification.getConfigurationFileType());
          } else {
            comJSONDisplay.printJSONDisplayFromText(
                comInput.getInput(), comSpecification.getTlvDisassemble());
          }
        } else {
          System.err.println(Input.ERROR);
          System.exit(1);
  ...
  public void decompile() {
    if (comInput != null && comInput.hasInput()) {
      if (comInput.isBinary()) {
        ...
        try {
          cf =
              new ConfigurationFile(
                  comSpecification.getConfigurationFileType(), cfe.getTlvBuilder());
        ...
        System.out.println(
            cfeSnmp64Insert.toPrettyPrint(
                comDecompile.checkVerbose(), comDecompile.checkSuppressed()));
      } else {
    ...
    if (comInput != null && comInput.hasInput()) {
      ...
      if (comInput.isBinary()) {
        ...
        try {
          cf =
              new ConfigurationFile(
                  cfe.getConfigurationFileType(), cfe.getTlvBuilder(), comKey.getKey());
        } catch (TlvException e) {
          e.printStackTrace();
        }
      } else {
        ...
        cf =
            new ConfigurationFile(
                comSpecification.getConfigurationFileType(), cfi.getTlvBuilder(), comKey.getKey());
      }
  ...
  public void bulkBuildCommand(File inputDirectory, File outputDirectory) {
    if (inputDirectory.isDirectory()) {
      if (outputDirectory.isDirectory()) {
        new BulkBuild(
                inputDirectory, outputDirectory, this.iBulkBuild, comKey.getKey(), tlvInsertion())
            .start();
      } else {
  ...
  public void bulkBuildCommand(File inputDirectory) {
    if (inputDirectory.isDirectory()) {
      new BulkBuild(
              inputDirectory,
              DirectoryStructure.fTimestampedDir(),
              this.iBulkBuild,
              comKey.getKey(),
              tlvInsertion())
          .start();
  ...
  public void tlvInsertion(ConfigurationFile configurationFile) {
    if (this.comFirmware != null && this.comFirmware.getFirmware() != null) {
      try {
        CommonTlvInsertions.insertFirmwareFileName(
            this.comFirmware.getFirmware(), configurationFile, false);
      } catch (ConfigurationFileException e) {
        e.printStackTrace();
      }
    }
    ...
    if (this.comTFTPServer != null) {
      if (this.comTFTPServer.hasIpv4Address()) {
        try {
          CommonTlvInsertions.insertTftpServerAddress(
              this.comTFTPServer.getIpv4Address(), configurationFile, false);
        } catch (ConfigurationFileException e) {
          e.printStackTrace();
        }
      }
      ...
      if (this.comTFTPServer.hasIpv6Address()) {
        try {
          CommonTlvInsertions.insertTftpServerAddress(
              this.comTFTPServer.getIpv6Address(), configurationFile, false);
        } catch (ConfigurationFileException e) {
          e.printStackTrace();
        }
      }
    ...
    if (this.comDownstreamFrequency != null
        && this.comDownstreamFrequency.getDownstreamFrequency() != -1) {
      try {
        CommonTlvInsertions.insertDownstreamFrequency(
            this.comDownstreamFrequency.getDownstreamFrequency(), configurationFile, false);
      } catch (ConfigurationFileException e) {
        e.printStackTrace();
      }
    }
      ...
      if (this.comCVC.hasManufacturer()) {
        try {
          CommonTlvInsertions.insertManufactureCVC(
              this.comCVC.getManufacturer(), configurationFile, false);
        } catch (ConfigurationFileException e) {
          e.printStackTrace();
        }
      }
    ...
    if (this.comDigitmapInsert != null) {
      for (Map.Entry<String, byte[]> entry : this.comDigitmapInsert.getDigitmap().entrySet()) {
        try {
          CommonTlvInsertions.insertDigitMap(
              entry.getValue(), entry.getKey(), configurationFile, false);
        } catch (ConfigurationFileException e) {
          e.printStackTrace();
        }
      }
    ...
    if (this.comOID != null) {
      for (Map.Entry<String, String[]> entry : this.comOID.getMap().entrySet()) {
        CommonTlvInsertions.insertSnmpOID(
            entry.getKey(),
            BERService.berStringDataTypeToByte(entry.getValue()[1]),
            entry.getValue()[0],
            configurationFile,
            false);
      }
  ...
  public CommonTlvInsertions tlvInsertion() {
    ...
    if (this.comDownstreamFrequency != null
        && this.comDownstreamFrequency.getDownstreamFrequency() != -1) {
      commonTlvInsertions.insertDownstreamFrequency(
          this.comDownstreamFrequency.getDownstreamFrequency(), false);
    }
