...
public class SqlConnection {
  ...
  public SqlConnection() {
    ...
    if (logger.isDebugEnabled()) logger.debug("SQLite DB Path: " + sCurrentPath);

    this.sqliteURL = "jdbc:Sqlite:" + sCurrentPath;

    if (sqlConnection == null) {

      if (logger.isDebugEnabled()) logger.debug("SqlConnection() -> First Time Connected to DB");

      connect();

    } else {

      if (logger.isDebugEnabled()) logger.debug("SqlConnection() -> Already Connected to DB");
    }
  }
  ...
  public static Integer getRowCount(ResultSet rsRsultSet) throws SQLException {
    ...
    while (rsRsultSet.next()) rowCount++;
  ...
  private void connect() {
    try {
      ...
      if (logger.isDebugEnabled()) logger.debug("SqlConnection.connect() -> URL: " + sqliteURL);

      sqlConnection = DriverManager.getConnection(sqliteURL, user, password);

      if (logger.isDebugEnabled()) logger.debug("SqlConnection.connect() -> Is Connected");
