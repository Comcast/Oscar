...
public class TlvAssembler extends TlvBuilder {
    ...
    for (int iJsonArrayIndex = 0; iJsonArrayIndex < jaTlvDictionary.length(); iJsonArrayIndex++) {
      ...
      try {
        ...
        if (joTlvDictionary.getBoolean(Dictionary.ARE_SUBTYPES)) {
          ...
          tbLocalBuilder.add(
              buildTlvFromTlvDictionary(joTlvDictionary.getJSONArray(Dictionary.SUBTYPE_ARRAY)));
  ...
  private TlvBuilder buildTlvFromTlvDictionary(JSONArray jaTlvDictionary) {
    ...
    for (int iJsonArrayIndex = 0; iJsonArrayIndex < jaTlvDictionary.length(); iJsonArrayIndex++) {
      ...
      try {
        ...
        if (joTlvDictionary.getBoolean(Dictionary.ARE_SUBTYPES)) {
          ...
          if (logger.isDebugEnabled())
            logger.debug(
                "SUB-TYPE-ARRAY-TRUE: -> Type: "
                    + joTlvDictionary.getString(Dictionary.TYPE)
                    + " -> "
                    + joTlvDictionary.getString(Dictionary.TLV_NAME));

          // If this is the start of the array, I need only advance 2 bytes to the next Type
          tbLocalTlvBuilder.add(
              buildTlvFromTlvDictionary(joTlvDictionary.getJSONArray(Dictionary.SUBTYPE_ARRAY)));
          ...
          if (tbLocalTlvBuilder.length() != 0) {
            ...
            if (logger.isDebugEnabled())
              logger.debug(
                  "TlvAssembler.buildTlvFromTlvDictionary:"
                      + " -> TLV-NAME: "
                      + joTlvDictionary.getString(Dictionary.TLV_NAME)
                      + " -> tbLocalTlvBuilder.length: "
                      + tbLocalTlvBuilder.length());
    ...
    if (logger.isDebugEnabled())
      logger.debug(
          "TlvAssembler.buildTlvFromTlvDictionary().tbLocalTlvBuilderFinal: "
              + tbLocalTlvBuilderFinal.toStringSeperation(":"));
  ...
  private TlvBuilder buildTlvFromTlvDictionary(JSONObject joTlvDictionary) throws JSONException {
    ...
    if (joTlvDictionary.has(Dictionary.VALUE)) {
      ...
      if (sDataType.equals(DataTypeDictionaryReference.DATA_TYPE_INTEGER)) {
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary().DATA_TYPE_INTEGER: "
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary().DATA_TYPE_OID: "
                  + tbLocalTlvBuilder.toString());

        // Convert JSONObject to Map
        // Map<String,Object> msoOidDataTypeValue =
        // JSONTools.toMap(joTlvDictionary.getJSONObject(Dictionary.VALUE));

        JSONArray ja = joTlvDictionary.getJSONArray(Dictionary.VALUE);

        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary()->ja.getJSONObject(0): "
        ...
        if (joTlvDictionary.getInt(Dictionary.BYTE_LENGTH) > 1) {
          ...
          try {
            tbLocalTlvBuilder.add(
                joTlvDictionary.getInt(Dictionary.TYPE), obcOidDataTypeValue.getBER());
          } catch (Exception e) {
            e.printStackTrace();
          }
        }
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary().DATA_TYPE_BYTE_ARRAY: "
                  + tbLocalTlvBuilder.toString());
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary().DATA_TYPE_STRING: "
                  + tbLocalTlvBuilder.toString());
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary() -> DATA_TYPE_MULTI_TLV_BYTE_ARRAY: "
                  + tbLocalTlvBuilder.toString());
        ...
        // Convert Integer to Byte Array
        HexString hsTlvValue =
            new HexString(DataTypeFormatConversion.binaryBitMaskToByteArray(sTlvValue));
        ...
        // Convert Integer to Byte Array
        HexString hsTlvValue =
            new HexString(DataTypeFormatConversion.inetAddressToByteArray(sTlvValue));
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary() -> DATA_TYPE_BYTE_ARRAY_IPV6_ADDR: "
                  + tbLocalTlvBuilder.toString());
        ...
        // Convert Integer to Byte Array
        HexString hsTlvValue =
            new HexString(DataTypeFormatConversion.ipv4TransportAddressToByteArray(sTlvValue));
        ...
        // Convert Integer to Byte Array
        HexString hsTlvValue =
            new HexString(DataTypeFormatConversion.ipv6TransportAddressToByteArray(sTlvValue));
        ...
        // Convert Integer to Byte Array
        HexString hsTlvValue =
            new HexString(DataTypeFormatConversion.inetTransportAddressToByteArray(sTlvValue));
        ...
        // Convert Integer to Byte Array
        HexString hsTlvValue =
            new HexString(DataTypeFormatConversion.macAddressFormatToByteArray(sTlvValue));
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary() -> DATA_TYPE_MAC_ADDRESS: "
                  + tbLocalTlvBuilder.toString());
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "TlvAssembler.buildTlvFromTlvDictionary() -> DATA_TYPE_OID_ASN1_OBJECT_6: "
                  + tbLocalTlvBuilder.toString());
      }
    }
